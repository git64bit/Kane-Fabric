...
class FirmwareAuthorityTests(unittest.TestCase):
    ...
    def test_inert_container_spec_is_pinned_and_host_mediated(self):
        ...
        self.assertEqual(2, spec["version"])
        ...
        self.assertEqual("default", spec["resources"]["storage_pool"])
        self.assertEqual("dir", spec["resources"]["storage_driver"])
        self.assertEqual("16GiB", spec["resources"]["root_disk_size_requested"])
        self.assertFalse(spec["resources"]["root_disk_quota_enforced"])
        self.assertIn("shared 935GiB", spec["resources"]["root_disk_quota_reason"])
        self.assertEqual("lxdbr0", spec["network"]["network"])
        self.assertEqual("dynamic", spec["network"]["address_assignment"])
