#!/usr/bin/env python3
"""Harvest, validate, and register a complete Kane Fabric official-building candidate."""
...
import kane_fabric_geometry as kane_geometry
import kane_fabric_provenance as kane_provenance
import kane_fabric_source_profiles as kane_source_profiles
import kane_fabric_source_status as kane_source_status
...
DEFAULT_ATTEMPTS = 3
USER_AGENT = "Kane-Fabric-Building-Candidate/1"
...
def load_building_profile() -> tuple[dict[str, Any], str]:
...
def normalize_feature_page(
    ...
    for index, feature in enumerate(payload["features"]):
        ...
        normalized_properties[identity_field] = stable_id
        normalized_feature = {
            "type": "Feature",
            "id": stable_id,
...
def _candidate_keys(source_published_at: str | None, content_sha256: str) -> tuple[str, str]:
    ...
    return (
        f"kane-buildings-candidate-{token}-{suffix}",
        f"kane-buildings-harvest-{token}-{suffix}",
    )
...
def harvest_candidate(
    ...
    *,
    ...
    if timeout_seconds <= 0:
        raise RuntimeError("timeout_seconds must be positive")
    profile, registry_sha256 = load_building_profile()
    ...
    for start in range(0, len(object_ids), page_size):
        ...
        normalized_features, normalized_pairs = normalize_feature_page(
            profile, page, page_ids, seen_stable_ids
        )
        features.extend(normalized_features)
        identity_pairs.extend(normalized_pairs)
    if len(features) != len(object_ids) or len(seen_stable_ids) != len(object_ids):
        raise HarvestContractError("Harvest completeness validation failed")
    ...
    if final_metadata_summary != metadata_summary:
        raise HarvestContractError("Layer metadata changed during the building harvest")
    ...
    if final_object_ids != object_ids:
        raise HarvestContractError("Object-ID inventory changed during the building harvest")
    ...
    buildings_root = staging_root / "buildings"
    buildings_root.mkdir(parents=True, exist_ok=True)
    if buildings_root.is_symlink():
        raise RuntimeError(f"Building staging directory must not be a symlink: {buildings_root}")
    final_dir = buildings_root / release_key
    if final_dir.exists():
    ...
    temporary: Path | None = Path(
        tempfile.mkdtemp(prefix=".building-candidate-", dir=buildings_root)
    )
    try:
        _write_file(temporary / "buildings.geojson", output_data)
        _write_file(temporary / "layer-metadata.json", metadata_data)
        _write_file(temporary / "object-ids.json", inventory_data)
        files = {
            "source": _file_record(temporary / "buildings.geojson"),
        ...
        manifest = {
            ...
            "candidate_kind": "official-buildings",
            "profile": {
                "profile_key": PROFILE_KEY,
...
def validate_candidate(
    ...
    collection, source_raw = load_canonical_json(candidate_dir / "buildings.geojson")
    collection = _require_mapping(collection, "Building candidate")
    if set(collection) != {"type", "name", "source", "features"}:
        raise RuntimeError("Building candidate has an unexpected key set")
    if collection.get("type") != "FeatureCollection" or collection.get("name") != PROFILE_KEY:
        raise RuntimeError("Building candidate has an invalid FeatureCollection identity")
    source = _require_mapping(collection.get("source"), "Building candidate source")
    expected_source = {
        "profile_key": PROFILE_KEY,
        "dataset_key": profile["dataset_key"],
        "layer_url": profile["source"]["layer_url"],
    ...
    if dict(source) != expected_source:
        raise RuntimeError("Building candidate source summary does not match metadata and profile")
    features = collection.get("features")
    if not isinstance(features, list) or len(features) != len(object_ids):
        raise RuntimeError("Building candidate feature count does not match object inventory")
    seen_stable: set[str] = set()
    identity_pairs: list[list[Any]] = []
    for index, (feature, expected_object_id) in enumerate(zip(features, object_ids)):
        normalized_features, pairs = normalize_feature_page(
            profile,
            {"type": "FeatureCollection", "features": [feature]},
            [expected_object_id],
            seen_stable,
        )
        if normalized_features[0] != feature:
            raise RuntimeError(f"Building candidate feature {index} is not normalized")
    ...
    if manifest.get("candidate_kind") != "official-buildings":
        raise RuntimeError("candidate_kind must be official-buildings")
    expected_profile = {
        "profile_key": PROFILE_KEY,
        "registry_filename": profile["registry_filename"],
        "registry_sha256": registry_sha256,
    ...
    if require_final_layout and (
        candidate_dir.name != release_key or candidate_dir.parent.name != "buildings"
    ):
        raise RuntimeError("Candidate directory must be buildings/RELEASE_KEY")
    return {
        "valid": True,
...
def _database_context(database: Path, profile: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if errors:
        raise RuntimeError("Database validation failed before candidate registration:\n- " + "\n- ".join(errors))
    ...
    try:
        ...
        if len(rows) != 1:
            raise RuntimeError(f"Buildings must have exactly one accepted release, found {len(rows)}")
        row = rows[0]
        if row["agency_key"] != profile["agency_key"]:
            raise RuntimeError("Accepted building agency does not match the approved profile")
        if row["data_kind"] != "buildings":
            raise RuntimeError("Accepted building dataset has an invalid data kind")
        if row["source_uri"] != profile["source"]["layer_url"]:
            raise RuntimeError("Accepted building source URI does not match the approved endpoint")
        return dict(row)
    finally:
        connection.close()
...
def _descriptor(
    ...
    files = []
    roles = {
        "buildings.geojson": ("source", "application/geo+json"),
    ...
    for name in sorted(roles):
        ...
        files.append(
            {
                "file_role": role,
                "relative_path": f"buildings/{validated['release_key']}/{name}",
                "byte_length": path.stat().st_size,
                "sha256": sha256_file(path),
...
def register_candidate(database: Path, candidate_dir: Path) -> dict[str, Any]:
    ...
    profile, _registry_sha256 = load_building_profile()
    ...
    if existing is not None:
        ...
        if _protected_counts(database) != protected_before:
            raise RuntimeError("Protected geographic identity state changed unexpectedly")
        return {
            "valid": True,
            "registered": True,
            "existing": True,
    ...
    handle, temporary_name = tempfile.mkstemp(
        prefix=".building-candidate-registration-", suffix=".json", dir=database.parent
    )
    try:
    ...
    if any(after[field] != context[field] for field in accepted_fields):
        raise RuntimeError("Accepted building release changed during candidate registration")
...
def candidate_info(database: Path, release_key: str) -> dict[str, Any]:
    ...
    if trace["dataset"]["dataset_key"] != "buildings":
        raise RuntimeError(f"Source release is not a building candidate: {release_key}")
    return {"valid": True, "candidate": trace}


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Harvest, validate, and register an official building candidate."
    )
    ...
    info = subparsers.add_parser("info", help="Trace one registered building candidate")
