#!/usr/bin/env python3
"""Check official Kane County source status for Kane Fabric."""
...
USER_AGENT = "Kane-Fabric-Source-Status/1"
DATASET_TABLES = {
...
def validate_layer_metadata(profile: Mapping[str, Any], metadata: Any) -> dict[str, Any]:
    ...
    if max_record_count is not None and (
    ...
    if isinstance(editing_info, Mapping):
        for key in edit_times:
            ...
            if isinstance(candidate, bool) or not isinstance(candidate, int) or candidate < 0:
                raise SourceUnexpectedError(f"editingInfo.{key} must be a nonnegative integer")
...
def _accepted_release(connection: sqlite3.Connection, dataset_key: str) -> sqlite3.Row:
    rows = connection.execute(
        ...
        "sr.feature_count, h.object_count FROM source_release sr "
...
def _accepted_object_ids(
    ...
    retained_count = connection.execute(
        f"SELECT COUNT(*) FROM {table} WHERE source_release_id = ?", (release_id,)
    ).fetchone()[0]
    ...
    try:
        ...
        for profile in registry["profiles"]:
            ...
            accepted_ids, limitation = _accepted_object_ids(
                connection, profile, int(release["source_release_id"]), feature_count, object_count
            )
...
def check_profile(
    ...
    *,
    ...
    try:
        ...
        reasons = _changed(accepted, live_ids, metadata_summary["comparison_edit_ms"])
        ...
        result.update(
            {"status": STATUS_UNAVAILABLE, "label": STATUS_LABELS[STATUS_UNAVAILABLE], "error": str(exc)}
        )
    except (SourceUnexpectedError, RuntimeError) as exc:
        result.update(
            {"status": STATUS_UNEXPECTED, "label": STATUS_LABELS[STATUS_UNEXPECTED], "error": str(exc)}
        )
    return result
