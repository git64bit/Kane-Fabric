#!/usr/bin/env python3
"""Import, validate, and inspect immutable Kane Fabric roads and water features."""
...
import kane_fabric_db as _fabric_db
...
class _DatabaseCompatibility:
    table_names = staticmethod(_fabric_db.table_names)
    valid_datetime = staticmethod(_fabric_db.valid_datetime)
    utc_now = staticmethod(_fabric_db.utc_now)

    @staticmethod
    def validate_database(path: Path) -> list[str]:
        return list(_fabric_db.validate_database(path)["errors"])


kane_db = _DatabaseCompatibility()
...
def validate_schema(connection: sqlite3.Connection) -> list[str]:
    ...
    expected_registration = ("features", "Kane Fabric roads and water", 4326)
