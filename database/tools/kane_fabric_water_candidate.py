#!/usr/bin/env python3
"""Harvest, validate, and atomically register the coordinated Kane Fabric water candidate."""
...
def register_candidate(database: Path, candidate_dir: Path) -> dict[str, Any]:
    ...
    if existing:
        ...
        if _protected_counts(database) != protected_before:
            raise WaterCandidateError("Protected geographic state changed unexpectedly")
        return {
