#!/usr/bin/env python3
"""Build and validate a reconciled Kane Fabric building candidate database."""
...
import kane_fabric_building_candidate as kane_candidate
import kane_fabric_buildings as kane_buildings
import kane_fabric_candidate_compare as kane_compare
import kane_fabric_db as kane_db
import kane_fabric_project_buildings as kane_project

RECONCILIATION_SCHEMA = 1
RECONCILIATION_DIRNAME = "reconciliation"
DATABASE_FILENAME = "kane-fabric-candidate.gpkg"
...
def _classification_snapshot(_connection: sqlite3.Connection) -> dict[str, Any]:
    empty_sha = sha256_value([])
    return {
        "current_count": 0,
        "event_count": 0,
...
def _classification_by_project(_connection: sqlite3.Connection) -> dict[int, str]:
    return {}
...
def build_plan(database: Path, candidate_dir: Path) -> dict[str, Any]:
    ...
    errors = kane_project.validate_database(database)
    if errors:
...
def _validate_candidate_database_with_current_migrations(database: Path) -> list[str]:
    ...
    errors = list(kane_project.validate_database(database))
    if not errors:
        return []
    try:
        connection = sqlite3.connect(f"file:{database.resolve()}?mode=ro", uri=True)
        ...
        try:
            ...
            pass
    expected = [
        (item.number, item.filename, item.sha256)
    ...
    with tempfile.TemporaryDirectory(prefix="kane-reconcile-validate-") as temp_root:
        ...
        return list(kane_project.validate_database(migrated))
...
def _build_report(
    ...
    if classifications_after != classifications_before:
        raise RuntimeError("Building reconciliation changed application-state snapshot")
    errors = kane_project.validate_database(candidate_database)
    if errors:
...
def validate_reconciliation(reconciliation_dir: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("Application-state preservation evidence is invalid")
