...
import kane_fabric_boundary_candidate as kane_boundary_candidate
import kane_fabric_building_reconcile as kane_reconcile
...
import kane_fabric_road_candidate as kane_road_candidate
import kane_fabric_water_candidate as kane_water_candidate

PROMOTION_SCHEMA = 1
ARTIFACT_DIRNAME = "promotion"
DATABASE_FILENAME = "kane-fabric-promoted.gpkg"
...
def prepare_promotion(
    ...
    try:
        ...
        if before_promotion["classifications"] != source_snapshot["classifications"]:
            raise RuntimeError("Candidate preparation changed application-state snapshot")
        ...
        if after["classifications"] != source_snapshot["classifications"]:
            raise RuntimeError("Promotion candidate changed application-state snapshot")
        database_info = {
...
def validate_promotion(directory: Path) -> dict[str, Any]:
    ...
    if manifest["promotion_key"] != f"kane-fabric-promotion-{manifest['promotion_plan_sha256'][:12]}":
...
def rollback_database(
    ...
    if current_snapshot["classifications"] != promoted_state["classifications"]:
        raise RuntimeError(
            "Rollback refused because application-state snapshot changed after promotion"
        )
