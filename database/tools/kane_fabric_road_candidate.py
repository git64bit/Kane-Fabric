#!/usr/bin/env python3
"""Harvest, validate, and register a complete Kane Fabric official-road candidate."""
...
DEFAULT_ATTEMPTS = 3
USER_AGENT = "Kane-Fabric-Road-Candidate/1"
...
def register_candidate(database: Path, candidate_dir: Path) -> dict[str, Any]:
    ...
    if existing is not None:
        ...
        if _protected_counts(database) != protected_before:
            raise RuntimeError("Protected geographic state changed unexpectedly")
        return {
            "valid": True,
            "registered": True,
            "existing": True,
