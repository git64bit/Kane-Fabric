#!/usr/bin/env python3
"""Check that Milestone 5 has one normative detailed work-sequence authority."""
...
DESIGN_RELATIVE = Path("docs/MILESTONE_5_DESIGN.md")
...
NORMATIVE_ITEM_RE = re.compile(r"^(MS5-\d{3})\b")
REFERENCE_ITEM_RE = re.compile(r"^\s*(?:[-*]\s+)?(MS5-\d{3})\b")
HISTORICAL_MILESTONE_RE = re.compile(r"^MILESTONE_[0-5]_")
...
def extract_normative_sequence(text: str) -> tuple[str, ...]:
    ...
    if not identifiers:
        raise RuntimeError("normative Milestone 5 work-sequence section contains no work items")
    if len(identifiers) != len(set(identifiers)):
        raise RuntimeError("normative Milestone 5 work-sequence contains a duplicate identifier")

    numbers = tuple(int(identifier.removeprefix("MS5-")) for identifier in identifiers)
    expected = tuple(range(1, max(numbers) + 1))
    if numbers != expected:
        raise RuntimeError(
            "normative Milestone 5 work-sequence identifiers must start at MS5-001 "
    ...
    try:
        ...
        if isinstance(current, int) and current > 5:
            ...
            if not isinstance(previous, dict):
                errors.append(
                    "docs/CURRENT_STATE.json must record released Milestone 5 in "
                    "milestone.previous_milestone after advancing beyond Milestone 5"
                )
                return
            if previous.get("number") != 5 or previous.get("status") != "released":
                errors.append(
                    "docs/CURRENT_STATE.json milestone.previous_milestone must identify "
                    "Milestone 5 as released after advancing beyond Milestone 5"
                )
            return

        errors.append(
            "docs/CURRENT_STATE.json milestone.current must be Milestone 5 or a later milestone"
        )
...
def validate(root: Path = ROOT) -> dict[str, object]:
    ...
    if sequence:
        for path in iter_current_markdown(root):
            ...
            if _contains_complete_identifier_set(identifiers, sequence):
                errors.append(
                    f"{relative} duplicates the complete normative Milestone 5 work sequence; "
    ...
    for relative in (ROADMAP_RELATIVE, HANDOFF_RELATIVE):
        ...
        if DESIGN_REFERENCE not in text:
            errors.append(
                f"{relative.as_posix()} must reference the Milestone 5 design authority "
                f"{DESIGN_REFERENCE}"
            )
